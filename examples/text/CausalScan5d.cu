...
#define SHAPE5D(t) {\
    (int)t.size(0), (int)t.size(1), (int)t.size(2), (int)t.size(3), (int)t.size(4), \
    (int)(t.size(1) * t.size(2) * t.size(3) * t.size(4)),\
    t.size(1) == 1 ? 0 : (int)(t.size(2) * t.size(3) * t.size(4)),\
...
#define SHAPE5D_Z(t) {\
    (int)t.size(0), (int)t.size(1), (int)t.size(2), (int)t.size(3), (int)t.size(4), \
    (int)(t.size(1) * t.size(2) * t.size(3) * t.size(4)),\
    (int)(t.size(2) * t.size(3) * t.size(4)),\
    (int)(t.size(3) * t.size(4)),\
    (scalar_t*)t.data_ptr()\
}
...
torch::Tensor causalScan5d_Forward(
    ...
    int range = (int)(S.size(0) * S.size(2) * S.size(3) * S.size(4));
    if(X.is_cuda()){
        #ifndef __DISABLE_CUDA__
        AT_DISPATCH_FLOATING_TYPES(X.scalar_type(), "causalScan5d_Forward", ([&] {
            ...
            device::causalScan5d_Forward_cuda<scalar_t><<<blocks, BLOCK_SIZE>>>(
        ...
        AT_DISPATCH_FLOATING_TYPES(X.scalar_type(), "causalScan5d_Forward", ([&] {
            ...
            wrap_t<scalar_t> shapeC = SHAPE5D(C);
            wrap_t<scalar_t> shapeO = SHAPE5D_Z(O);
            at::parallel_for(0, shapeZ.b*shapeZ.h*shapeZ.d, 0, [&](int64_t start, int64_t end){
                while(start<end){
...
std::vector<torch::Tensor> causalScan5d_Backward(
    ...
    if(gradO.is_cuda()) {
        #ifndef __DISABLE_CUDA__
        AT_DISPATCH_FLOATING_TYPES(X.scalar_type(), "causalScan5d_Backward", ([&] {
            wrap_t<scalar_t> shapeX = SHAPE5D_Z(X);
            ...
            device::causalScan5d_Backward_cuda<scalar_t><<<blocks, BLOCK_SIZE>>>(
