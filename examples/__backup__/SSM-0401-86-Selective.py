...
def SSMBlock(**kwargs):
    ...
    def __init__(self, **kwargs):
        ...
        # rkv, q, k, v, gh, go
        self.in_proj = nn.Linear(args.latent_dim, 2 * self.num_heads + self.hidden_dim + self.gh_dim + self.go_dim, bias=args.bias)
    ...
    def forward(self, x, state=None, **kwargs):
        (b, l, d) = x.shape
        (rkv, ig, v, gh, go) = self.in_proj(x).split([
        ...
        # C = np.index_select(q, dim=2, index=idx)                        #q:[b,l,h,k], C:[b,l,h,k]
        idx.unsqueeze(-1)
        ...
        shiftB = B[:l-1]
        shiftV = v[:,:l-1] / (1-10+cumA[:,:l-1]).unsqueeze(-1)
