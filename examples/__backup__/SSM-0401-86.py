...
def SSMBlock(**kwargs):
    ...
    def __init__(self, **kwargs):
        ...
        # rkv, q, k, v, gh, go
        self.in_proj = nn.Linear(args.latent_dim, 2 * self.num_states * self.num_heads + self.num_heads + self.hidden_dim + self.gh_dim + self.go_dim, bias=args.bias)
    ...
    def forward(self, x, state=None, **kwargs):
        ...
        (t,s0) = ssm_state if ssm_state is not None else (
            0,  # t
            np.zeros(b, 1, self.num_heads, self.num_states, d//self.num_heads, device=x.device)
        )
        ...
        k = np.rearrange('(b l) h (k v)->b l h k v', B, b=1, v=1)
        v = np.rearrange('b l (h k v)->b l h k v', v, h=self.num_heads, k=1) * np.sigmoid(ig).unsqueeze(-1).unsqueeze(-1)
        kv = (1-rkv)*k*v
        ...
        q = np.rearrange('(b l) h (k v)->b l h k v', C, b=1, v=1)
