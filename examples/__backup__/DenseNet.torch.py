...
class DenseNet(nn.Module):
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_channels=3, num_classes=1000, num_layers=121):
        super(DenseNet, self).__init__()

        name = "densenet"+str(num_layers)
        if(model_cfgs[name] == None):
            name="densenet121"
    
        block_config = model_cfgs[name]
